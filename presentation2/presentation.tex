...
\begin{itemize}
  ...
  \item States: \texttt{EMPTY}, \texttt{PROLIFERATING}, \texttt{QUIESCENT}, \texttt{NECROTIC}, \texttt{DEAD}.
...
\[
\begin{cases}
\text{ANN forward} \rightarrow \text{action }(P,Q,A),\\
\text{age update with }F(R),\\
\text{apoptosis / necrosis / proliferation rules},\\
c^{t+\Delta t}=c^t+\Delta t\big(D_c\Delta c-\alpha\big),\; c\ge 0.
\end{cases}
\]
\end{block}
\end{frame}
